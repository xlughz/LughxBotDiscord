  ...
  static async banUser({
    ...
    try {
      ...
      if (targetMember) {
        ...
        if (!botCheck.valid) {
          throw new LughxBotError(botCheck.error, ErrorTypes.PERMISSION, botCheck.error);
        }

        const modCheck = this.validateHierarchy(moderator, targetMember, 'ban');
        if (!modCheck.valid) {
          throw new LughxBotError(modCheck.error, ErrorTypes.PERMISSION, modCheck.error);
        }
      } else {
